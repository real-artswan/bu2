    ...
    public class BaboNetPacketProcessor
    {
        ...
        {   
			...
			if (!gameState.map.mapCreated) {
				...
				request.mapName = parsedPacket.mapName;
		...
		{
			if (parsedPacket.frameID > serverFrameID)
			{
				serverFrameID = parsedPacket.frameID;
				gameState.gameTimeLeft = parsedPacket.gameTimeLeft;
				gameState.hud.roundTimer.text = parsedPacket.roundTimeLeft.ToString();
			}
		}
    }
}
