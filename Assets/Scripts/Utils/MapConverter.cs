        ...
        public static void CreateUnityMap(BaboMap baboMap, Map mapObject) {
                ...
                for (int wInd = 0; wInd < baboMap.width; wInd++) {
                    ...
                    if (!cell.passable) {
                        ...
                        wall.transform.position = new Vector3(
                            wInd - wShiftWall,
                            cell.height / 2f, hInd - hShiftWall);
                        wall.transform.localScale = new Vector3(1, cell.height, 1);
                        wall.GetComponent<Renderer>().material.mainTexture = wallTex;
                        if (cell.height != 1)
                            BaboUtils.fixScaledMeshUVs(
                                wall.GetComponent<MeshFilter>().mesh,
            ...
            foreach (Vector3 spawn in baboMap.dm_spawns) {
                ...
                spawnObject.transform.position = new Vector3(
                    spawn.x - wShiftWall - 0.5f,
                    spawn.z,
                    spawn.y - hShiftWall - 0.5f);
            ...
            Vector3 p = new Vector3(
                baboMap.blueFlagPodPos.x - wShiftWall - 0.5f,
                baboMap.blueFlagPodPos.z,
                baboMap.blueFlagPodPos.y - hShiftWall - 0.5f);
            ...
            p = new Vector3(
                baboMap.redFlagPodPos.x - wShiftWall - 0.5f,
                baboMap.redFlagPodPos.z,
                baboMap.redFlagPodPos.y - hShiftWall - 0.5f);
            ...
            //objectives (??)
            p = new Vector3(
                baboMap.blueObjective.x - wShiftWall - 0.5f,
                baboMap.blueObjective.z,
                baboMap.blueObjective.y - hShiftWall - 0.5f);
            GameObject obj = new GameObject("BlueObjective");
            obj.transform.parent = mapObject.gameObject.transform;
            obj.transform.position = p;

            p = new Vector3(
                baboMap.redObjective.x - wShiftWall - 0.5f,
                baboMap.redObjective.z,
                baboMap.redObjective.y - hShiftWall - 0.5f);
        ...
        private static void GenerateGroundMesh(GameObject floor, Material[] materials, BaboMap baboMap) {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    colors[ind] = new Color32(255, 255, 255,
                        (byte)(255 - baboMap.cells[ind].splater[1] * 255));
